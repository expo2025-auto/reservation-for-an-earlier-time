...
(function () {
  ...
  const DISPLAY_SETTLE_AFTER_LOAD_MS = 2_500;
  const DISPLAY_SETTLE_AFTER_CHECK_MS = 1_200;
  ...
  let lastReloadSettleLogBucket = null;
  ...
  function isDisplaySettled() {
    const now = Date.now();
    if (pageLoadTime > 0 && now - pageLoadTime < DISPLAY_SETTLE_AFTER_LOAD_MS) {
      return false;
    }
    if (lastSuccessfulCheckTime <= 0 || lastSuccessfulCheckTime < pageLoadTime) {
      return false;
    }
    if (now - lastSuccessfulCheckTime < DISPLAY_SETTLE_AFTER_CHECK_MS) {
      return false;
    }
    return true;
  }

  function computeDisplaySettleDelay() {
    if (isDisplaySettled()) {
      return 0;
    }
    const now = Date.now();
    const waitTargets = [];
    if (pageLoadTime > 0) {
      waitTargets.push(pageLoadTime + DISPLAY_SETTLE_AFTER_LOAD_MS);
    }
    if (lastSuccessfulCheckTime > 0 && lastSuccessfulCheckTime >= pageLoadTime) {
      waitTargets.push(lastSuccessfulCheckTime + DISPLAY_SETTLE_AFTER_CHECK_MS);
    }
    if (!waitTargets.length) {
      return DISPLAY_SETTLE_POLL_INTERVAL_MS;
    }
    const waitUntil = Math.max(...waitTargets);
    return Math.max(DISPLAY_SETTLE_POLL_INTERVAL_MS, waitUntil - now);
  }
  ...
  function scheduleReload(reason = '', delay = 600) {
    ...
    pendingReload = true;
    const finalReason = reason || '自動リロード';
    if (reason) log(`リロード予約: ${reason}`);
    const attemptReload = () => {
      const settleDelay = computeDisplaySettleDelay();
      if (settleDelay > 0) {
        setTimeout(attemptReload, settleDelay);
        return;
      }
      reloadPage(finalReason);
    };
    const initialDelay = Math.max(0, delay, computeDisplaySettleDelay());
    setTimeout(attemptReload, initialDelay);
  }
  ...
  function reloadPage(reason = '') {
    ...
    if (typeof location.reload === 'function') {
      try {
        ...
        log('location.reload の呼び出しで例外が発生しました。location.replace でリトライします。');
      }
    }
  ...
  async function tick() {
    ...
    try {
      ...
      if (inWindow && reloadsThisMinute < MAX_RELOADS_PER_MINUTE) {
        ...
        if (!isDisplaySettled()) {
          if (lastReloadSettleLogBucket !== bucket) {
            log('画面表示の更新完了待ちのためリロードを一時停止します');
            lastReloadSettleLogBucket = bucket;
          }
          return;
        }
        lastReloadSettleLogBucket = null;
        reloadsThisMinute++;
        reloadInfo.count = reloadsThisMinute;
        saveReloadInfo(reloadInfo);
        scheduleReload(`サーバー時刻 ${sec}s（分内 ${reloadsThisMinute}/${MAX_RELOADS_PER_MINUTE}）`, 0);
        return;
      }
      if (!inWindow) {
        lastReloadDeferLogBucket = null;
